...
<template>
  <div class="app-container">
    ...
    <div v-else class="specimen-grid">
      ...
      >
        <div class="card-image">
          ...
          <div v-else class="image-placeholder"><el-icon :size="32"><Picture /></el-icon></div>
          <!-- 待审核图片数量徽章 -->
    ...
    <!-- ========== 标本详情弹窗 ========== -->
    <el-dialog v-model="detailVisible" :title="currentSpecimen?.specimenName" width="800px" :close-on-click-modal="false">
      <div class="detail-grid">
            ...
            <div v-for="img in currentSpecimen?.images" :key="img.imageId" class="image-item">
              ...
              <div class="image-info">
                ...
                <div class="image-status">
                  ...
                  <el-tag v-else-if="img.auditStatus === '0'" type="warning" size="small">待审核</el-tag>
                  <el-tag v-else-if="img.auditStatus === '2'" type="danger" size="small">已拒绝</el-tag>
                </div>
              </div>
              <div class="image-actions" v-if="isAdmin">
                <template v-if="img.auditStatus === '0'">
                  <el-button size="small" type="success" @click="auditImage(img, '1')">通过</el-button>
                  <el-button size="small" type="danger" plain @click="auditImage(img, '2')">拒绝</el-button>
                </template>
        ...
        <div class="detail-info">
          ...
          <div class="info-row">
            ...
            <span class="info-value">{{ currentSpecimen?.category?.name || '-' }}</span>
          </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="danger" plain @click="confirmDeleteSpecimen" v-hasPermi="['admin:specimen:remove']">
            <el-icon><Delete /></el-icon>
            删除此标本
          </el-button>
    ...
    <el-dialog v-model="formVisible" :title="editSpecimen ? '编辑标本' : '新增标本'" width="500px" :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="分类" prop="categoryId">
          <el-select v-model="form.categoryId" placeholder="选择分类" style="width: 100%" :disabled="!form.museumId">
            <el-option v-for="c in formCategoryOptions" :key="c.id" :label="c.name" :value="c.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="拉丁学名" prop="latinName">
          <el-input v-model="form.latinName" placeholder="如：Panax ginseng C. A. Mey." style="font-style: italic" />
        </el-form-item>
        <el-form-item label="备注信息" prop="remark">
          <el-input v-model="form.remark" type="textarea" :rows="3" placeholder="标本详细描述、功效等信息..." />
        </el-form-item>
      </el-form>
      <template #footer>
...
const isAdmin = computed(() => userStore.roles?.includes('ROLE_ADMIN') || userStore.permissions?.includes('admin:specimen:edit'))
...
function onFormMuseumChange(museumId: number | undefined) {
  ...
  if (museumId) {
    const museum = museumOptions.value.find(m => m.id === museumId)
