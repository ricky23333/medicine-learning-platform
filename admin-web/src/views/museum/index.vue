...
<template>
  <div class="app-container">
    ...
    <div class="museum-list" v-if="museumList && museumList.length !== 0">
      <div v-for="museum in museumList" :key="museum.museumId" class="museum-card">
        <!-- 馆头部 -->
        <div
          class="museum-header"
        ...
        <div v-if="expandedMuseum === museum.museumId" class="category-section">
          <div class="category-header">
            ...
            <el-button size="small" type="success" plain @click="openAddCategory(museum.museumId)">
              <el-icon><Plus /></el-icon>
              添加分类
            </el-button>
          </div>
          ...
          <div v-else class="category-grid">
            <div v-for="cat in museum.categories" :key="cat.categoryId" class="category-item">
              ...
              <div class="category-actions">
                <el-button text size="small" @click="openEditCategory(museum.museumId, cat)">
                  <el-icon><Edit /></el-icon>
                </el-button>
                <el-button
                  text
    ...
    <el-dialog v-model="deleteDialogVisible" title="确认删除" width="400px">
      <div class="delete-confirm">
        ...
        <p class="delete-title">
          确定要删除{{ deleteTarget.type === 'museum' ? '标本目录' : '分类' }}「{{
            deleteTarget.name
          }}」吗？
        </p>
  ...
  async function loadMuseums() {
    try {
      ...
      if (res.code === 200 && res.data) {
        ...
        museumList.value.forEach((m: any) => {
          let museumSpecimenCount = 0
          if (m.categories) {
            m.categories.forEach((c: any) => {
              museumSpecimenCount += c.specCount
              categorySpecimenCountMap.value[c.categoryId] = c.specCount
            })
          }
          specimenCountMap.value[m.museumId] = museumSpecimenCount
        })
      }
    } catch (error) {
