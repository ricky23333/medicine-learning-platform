  ...
  <div class="app-container">
    ...
    <div class="museum-list">
      <div v-for="museum in museumList" :key="museum.id" class="museum-card">
        <!-- 馆头部 -->
        <div class="museum-header" :class="{ expanded: expandedMuseum === museum.id }" @click="toggleMuseum(museum.id)">
          <div class="museum-icon">{{ museum.icon || '🏛️' }}</div>
          <div class="museum-info">
            <div class="museum-name-row">
              <span class="museum-name">{{ museum.name }}</span>
              <el-tag size="small" type="success">{{ museum.categories?.length || 0 }}个分类</el-tag>
              <el-tag size="small" type="info">{{ getSpecimenCount(museum.id) }}个标本</el-tag>
            </div>
          ...
          <div class="museum-actions" @click.stop>
            ...
            <el-icon class="expand-icon" :class="{ rotated: expandedMuseum === museum.id }">
              <ArrowRight />
            </el-icon>
          </div>
        </div>
        ...
        <div v-if="expandedMuseum === museum.id" class="category-section">
          <div class="category-header">
            ...
            <el-button size="small" type="success" plain @click="openAddCategory(museum.id)">
              <el-icon><Plus /></el-icon>
              添加分类
            </el-button>
          </div>

          <div v-if="!museum.categories || museum.categories.length === 0" class="category-empty">
            <el-icon :size="32"><Folder /></el-icon>
            <p>暂无分类，请添加</p>
          </div>

          <div v-else class="category-grid">
            <div v-for="cat in museum.categories" :key="cat.id" class="category-item">
              ...
              <div class="category-info">
                ...
                <div class="category-meta">
                  {{ getCategorySpecimenCount(cat.id) }}个标本 · {{ cat.description || '无描述' }}
                </div>
              </div>
              <div class="category-actions">
                <el-button text size="small" @click="openEditCategory(museum.id, cat)">
                  <el-icon><Edit /></el-icon>
                </el-button>
                <el-button text size="small" type="danger" @click="confirmDeleteCategory(museum.id, cat)">
                  <el-icon><Delete /></el-icon>
                </el-button>
              </div>
            </div>
    ...
    <el-dialog v-model="museumDialogVisible" :title="editMuseum ? '编辑标本目录' : '新增标本目录'" width="500px" :close-on-click-modal="false">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveMuseum">{{ editMuseum ? '保存' : '创建' }}</el-button>
      </template>
    </el-dialog>

    <!-- 新增/编辑分类弹窗 -->
    <el-dialog v-model="categoryDialogVisible" :title="editCategory ? '编辑分类' : '添加分类'" width="500px" :close-on-click-modal="false">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveCategory">{{ editCategory ? '保存' : '添加' }}</el-button>
      </template>
    </el-dialog>
