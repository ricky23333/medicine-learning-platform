        ...
        <div class="h-100 df fdc">
          ...
          >
            ...
            <el-form-item label="专业年级" prop="majorGrade">
              <el-input
                v-model="queryParams.majorGrade"
                placeholder="请输入专业年级"
                clearable
                style="width: 207px"
                @keyup.enter="handleQuery"
          ...
          <div class="f1">
            ...
            >
              ...
              <template #userType="{ row }">
                <el-tag v-if="row.appUser?.userType === 'teacher'" type="success">教师</el-tag>
                <el-tag v-else-if="row.appUser?.userType === 'student'" type="primary">学生</el-tag>
                <span v-else>-</span>
              </template>
              ...
              <template #operate="{ row }">
                ...
                <el-tooltip
                  content="升级为VIP"
                  placement="top"
                  v-if="row.userId !== 1 && isTeacherNonVip(row)"
                >
                  <el-button
                    link
                    type="primary"
                    icon="Star"
                    @click="handleUpgradeVip(row)"
                    v-hasPermi="['system:user:edit']"
                  ></el-button>
                </el-tooltip>
              </template>
...
<script lang="jsx" setup name="User">
  ...
  const columns = shallowReactive([
    ...
    {
      prop: 'appUser.majorGrade',
      label: '专业年级',
      minWidth: 120
    },
    {
      prop: 'appUser.studentNo',
      label: '学号',
      minWidth: 100
    },
    {
  ...
  const data = reactive({
    ...
    rules: {
      ...
      deptId: [{ required: true, message: '请选择归属部门', trigger: ['blur', 'change'] }],
  ...
  /** 判断是否为教师且非VIP */
  function isTeacherNonVip(row) {
    return row.appUser && row.appUser.userType === 'teacher' && row.appUser.vipStatus !== '2'
  }

  /** 升级为VIP按钮操作 */
  function handleUpgradeVip(row) {
    proxy.$modal
      .confirm('确认要将教师"' + row.userName + '"升级为VIP吗？')
      .then(function () {
        return upgradeToVip(row.userId)
      })
      .then(() => {
  ...
  function handleDisable() {
    proxy.$modal
      .confirm('确认要停用选中的' + ids.value.length + '个用户吗？')
      .then(function () {
        return changeUserStatus(null, '1', ids.value)
      })
      .then(() => {
  ...
  function handleEnable() {
    proxy.$modal
      .confirm('确认要启用选中的' + ids.value.length + '个用户吗？')
      .then(function () {
        return changeUserStatus(null, '0', ids.value)
      })
      .then(() => {
  ...
  function resetQuery() {
    ...
    queryParams.value.majorGrade = undefined
